...
def Resnet_model_fn(features, labels, mode):
    ...
    model = ResNetModel(config.block_sizes, config.num_classes, data_format = "channel_first")
    ...
    cross_entry = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = logits, labels = labels)
    tf.summary.scalar("cross entry", cross_entry)
    vars = tf.trainable_variables()
    l2_loss = tf.add_n([ tf.nn.l2_loss(v) for v in vars ]) * config.weight_decay
    loss = cross_entry + l2_loss
    ...
    training_hook = tf.train.LoggingTensorHook({'loss' : loss, 'accuracy' : accuracy}, every_n_iter = config.log_frequency)
...
def run():
    ...
    for cycle_index, num_train_epochs in enumerate(schedule):
        ...
        if num_train_epochs:
            classifier.train(input_fn = lambda : input_function("train", config.train_dir, config.train_batch_size, num_train_epochs))
        tf.logging.info("start evaluate")
        eval_results = classifier.eval(input_fn = lambda : input_function("eval", config.eval_dir, 1, 1))
        print(eval_results)
