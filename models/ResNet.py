...
def Conv2d_pading(inputs, filters, kernel_size, strides, data_format):
    ...
    return tf.layers.conv2d(inputs, filters, kernel_size, strides, padding = ('SAME' if strides == 1 else 'VALID'), data_format = data_format, use_bias = False, kernel_initializer = tf.variance_scaling_initializer())
    ...
    def __call__(self, inputs, training):
        ...
        with tf.variable_scope("resnet50"):
            ...
            for i, block_nums in enumerate(self.block_sizes):
                inputs = make_layer(inputs, filters = 64 * (2**i), strides = BlockStrides[i], blocks = block_nums, name = "block_layer{}".format(i+1), training = training, data_format = self.data_format)
